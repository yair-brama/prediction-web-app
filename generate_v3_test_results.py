...
CALIBRATOR_FILE = os.path.join(BASE_DIR, "lgbm_cards_lag_calibrator.joblib")
OUTPUT_FILE    = os.path.join(BASE_DIR, "v3_lag_model_test_results.xlsx")
...
def main():
    # ------------------------------------------------------------------
    # 1. Load data (same steps as train_lag_model.py)
    # ------------------------------------------------------------------
    print("Loading data...")
    ...
    # ------------------------------------------------------------------
    # 2. Feature engineering (V3: calendar + holiday + lag)
    # ------------------------------------------------------------------
    ...
    # Lag / Rolling features (same logic as train_lag_model.py)
    ...
    # ------------------------------------------------------------------
    # 3. Reproduce the same 70/10/20 split as training
    # ------------------------------------------------------------------
    q80 = df["Date"].quantile(0.80)
    test_mask = df["Date"] > q80

    X_test = df.loc[test_mask, FEATURES_V3]
    ...
    # ------------------------------------------------------------------
    # 4. Load model + calibrator and predict
    # ------------------------------------------------------------------
    print("Loading model and predicting...")
    booster = lgb.Booster(model_file=MODEL_FILE)
    calibrator = joblib.load(CALIBRATOR_FILE)

    raw_preds = booster.predict(X_test, num_iteration=booster.best_iteration)
    y_pred = calibrator.predict(raw_preds)
    y_pred = np.maximum(y_pred, 0)  # ensure non-negative

    # ------------------------------------------------------------------
    # 5. Build the row-level results DataFrame
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # 6. Build the per-location summary
    # ------------------------------------------------------------------
    def loc_summary(g):
        actual = g["Actual"]
        pred   = g["Predicted"]
    ...
    print(f"\n  Overall Test Metrics (V3 Lag Model):")
